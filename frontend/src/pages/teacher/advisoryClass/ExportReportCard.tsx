...
type jsPDFCustom = jsPDF & { lastAutoTable: { finalY: number } };
...
function computeFinal(
  ...
  const nums = [sem1, sem2, sem3].filter(
...
export function buildCoreValuesBody(
  coreValues: Record<string, Record<string, string[]>> = {},
): any[][] {
...
export async function generateSF9PDF(params: {
  ...
  if (!loadedStudent && sid) {
    try {
      if (queryClient) {
        loadedStudent = await queryClient.fetchQuery({
          queryKey: ["student", "detail", sid],
          queryFn: () => getStudentDetail(sid),
          staleTime: 5 * 60 * 1000,
        });
      } else {
        loadedStudent = await getStudentDetail(sid);
      }
    } catch (e) {
      console.warn("Failed to fetch student details for SF9:", e);
    }
  }
  ...
  if (!loadedGrades && sid) {
    try {
      if (queryClient) {
        loadedGrades = await queryClient.fetchQuery({
          queryKey: ["teacher", "advisory", "student", sid, "semester-summary"],
          queryFn: () => getStudentSemesterSummary(sid),
          staleTime: 5 * 60 * 1000,
        });
      } else {
        loadedGrades = await getStudentSemesterSummary(sid);
      }
    } catch (e) {
      console.warn("Failed to fetch semester summary for SF9:", e);
    }
  }
  ...
  const generalAverage =
    finals.length > 0 ? finals.reduce((a, b) => a + b) / finals.length : null;
      ...
      [
        {
          ...
          rowSpan: 2,
          styles: { halign: "justify", valign: "middle", fontSize: 12 },
        },
        {
          content: "SEMESTER",
          colSpan: 3,
  ...
  autoTable(pdf, {
    ...
    body: [
      [
        "No. of School Days",
        ...att.schoolDays.map((v) => (v > 0 ? String(v) : "")),
        att.schoolDays.reduce((a, b) => a + b, 0),
      ],
      [
        "No. of Days Present",
        ...att.present.map((v, i) => (att.schoolDays[i] > 0 ? String(v) : "")),
        att.present.reduce((a, b) => a + b, 0),
      ],
      [
        "No. of Times Absent",
        ...att.absent.map((v, i) => (att.schoolDays[i] > 0 ? String(v) : "")),
        att.absent.reduce((a, b) => a + b, 0),
      ],
    ],
    theme: "grid",
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(10);
  pdf.text("Republic of the Philippines", centerX + 200, 65, {
    align: "center",
  });
  ...
  pdf.setFont("helvetica", "bold");
  pdf.setFontSize(12);
  pdf.text("LEARNER'S PROGRESS REPORT CARD", centerX + 200, 190, {
    align: "center",
  });
  pdf.setFontSize(10);
  pdf.text(
  ...
  pdf.text("Head Teacher / Principal", rightColX + 75, 420, {
    align: "center",
  });
  ...
  pdf.setFontSize(8);
  pdf.text(
    "Admitted to Grade: ________ Section: ________ Room: ________",
    rightColX,
    470,
  );
  pdf.text(
    "Eligible for Admission to Grade: _____________________________",
    rightColX,
    485,
  );
  ...
  pdf.text("Head Teacher / Principal", rightColX + 70, 540, {
    align: "center",
  });
  ...
  pdf.save(
    `SF9_${lrn || loadedStudent?.school_id || "student"}_${loadedStudent?.last_name || ""}.pdf`,
  );
}
...
export default function ExportReportCardPDF(): JSX.Element {
  ...
  const {
    ...
    error: gradesError,
  } = useStudentSemesterSummary(sid, sid > 0);

  const loading = studentLoading || gradesLoading;
  const errorMsg = isStudentError
    ? studentError instanceof Error
      ? studentError.message
      : "Failed to load student details."
    : isGradesError
      ? gradesError instanceof Error
        ? gradesError.message
  ...
  const coreValuesBody = useMemo(
    () => buildCoreValuesBody(coreValues),
    [coreValues],
  );
          ...
          <div className="flex items-center justify-between mb-4 px-2">
            <div className="flex flex-col">
              ...
              <p className="text-[10px] text-slate-500 font-medium">
                Draft generated from backend semester grades
              </p>
            </div>
                      ...
                      <thead>
                        ...
                        <tr className="font-bold text-center">
                          {["1", "2", "3"].map((s) => (
                            <th
                              key={s}
                              className="border border-black p-1 w-10"
                            >
                              {s}
                            </th>
                          ))}
                      ...
                      <tbody>
                        ...
                        <tr className="font-bold bg-slate-50">
                          ...
                          <td className="border border-black p-1 text-center underline">
                            {generalAverage != null
                              ? generalAverage.toFixed(3)
                              : ""}
                          </td>
                          <td
                            className={`border border-black p-1 text-center uppercase ${
                              generalAverage != null && generalAverage >= 75
                                ? "text-emerald-700"
                                : "text-red-700"
                            }`}
                          >
                            ...
                            <React.Fragment key={idx}>
                              {val.statements.map((stmt, sIdx) => (
                                <tr key={sIdx}>
                                  {sIdx === 0 && (
                                    <td
                                      className="border border-black p-2 font-bold w-32"
                                  ...
                                  {[
                                    ...
                                    <td
                                      key={semKey}
                                      className="border border-black p-1 text-center font-medium w-8"
                                    >
                                      {coreValues[semKey]?.[val.value]?.[
                                        sIdx
                                      ] || ""}
                                    </td>
                                  ))}
                ...
                <div className="grid grid-cols-2 gap-12 h-full">
                  ...
                  <div className="border-r border-slate-100 pr-12">
                    ...
                    <table className="w-full border-[1.5px] border-black text-[9px] border-collapse text-center">
                      <thead>
                        <tr className="font-bold">
                          <th className="border border-black p-1 text-left bg-slate-50">
                            Month
                          </th>
                          {MONTHS.slice(1).map((m) => (
                            <th key={m} className="border border-black p-1">
                              {m}
                            </th>
                          ))}
                      ...
                      <tbody>
                        <tr>
                          <td className="border border-black p-2 text-left font-bold bg-slate-50">
                            No. of School Days
                          </td>
                    ...
                    <div className="mt-16">
                      ...
                      <div className="space-y-8">
                        {["1st Semester", "2nd Semester", "3rd Semester"].map(
                          (s) => (
                            <div key={s} className="flex items-center gap-4">
                              <span className="text-[10px] font-bold w-24">
                                {s}:
                              </span>
                  ...
                  <div className="flex flex-col items-center px-8">
                    ...
                    <div className="w-full space-y-5 text-[12px] mt-6">
                      ...
                      <div className="flex border-b border-black pb-1">
                        ...
                        <span>
                          {manualStudent.lrn ?? student?.school_id ?? "—"}
                        </span>
                      </div>
                    </div>
                    ...
                    <img
                      src={deped_logo}
                      alt="DepEd"
                      className="w-20 h-20 object-contain mt-10 opacity-80"
                    />
                  </div>
                </div>
              )}
